...
function buildFallbackTicket(subject: string, message: string, userId: string | null) {
  return {
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      data: [buildFallbackTicket(subject, message, userId)],
      fallback: true,
    })
  } catch (err) {
    ...
    return NextResponse.json(
      {
        data: [buildFallbackTicket("Purchase Request", "Fallback submission", null)],
        fallback: true,
      },
      { status: 200 },
    )
